package com.ultreon.mods.masterweapons.worldgen;
...
public class WorldGeneration {
    ...
    public static void generate(BiomeLoadingEvent event) {
        if (!initialized) {
            ImmutableList<OreConfiguration.TargetBlockState> targets = ImmutableList.of(OreConfiguration.target(OreConfiguration.Predicates.STONE_ORE_REPLACEABLES, ModBlocks.MASTER_ORE.get().defaultBlockState()), OreConfiguration.target(OreConfiguration.Predicates.DEEPSLATE_ORE_REPLACEABLES, ModBlocks.DEEPSLATE_MASTER_ORE.get().defaultBlockState()));
...
//                    .withPlacement(Placement.CHANCE.configure(new ChanceConfig(64)));
            feature = Feature.REPLACE_SINGLE_BLOCK
                    .configured(new ReplaceBlockConfiguration(targets)).rangeUniform(VerticalAnchor.bottom(), VerticalAnchor.absolute(15))
                    .count(16)
//                    .count(1)
